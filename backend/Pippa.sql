...
CREATE TABLE `stock_item_map_kit` (
  `id` UUID PRIMARY KEY,
  `kit_id` UUID NOT NULL,
  `component_stock_item_id` UUID NOT NULL,
  `requered_quantity` INT NOT NULL DEFAULT 1,
  `display_order` INT NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX `unique_kit` ON `kit_info` (`belongs_to`, `stock_item_id`);

CREATE UNIQUE INDEX `unique_kit_component` ON `stock_item_map_kit` (`kit_id`, `component_stock_item_id`);

CREATE INDEX `idx_kit_display_order` ON `stock_item_map_kit` (`kit_id`, `display_order`);
...
CREATE UNIQUE INDEX `bc_categories_index_15` ON `bc_categories` (`store_id`, `bc_category_id`);

CREATE UNIQUE INDEX `bc_products_index_16` ON `bc_products` (`store_id`, `sku`);

CREATE INDEX `bc_products_index_17` ON `bc_products` (`store_id`);

CREATE INDEX `bc_products_index_18` ON `bc_products` (`sku`);

CREATE INDEX `bc_products_index_19` ON `bc_products` (`bc_product_id`);

CREATE INDEX `bc_products_index_20` ON `bc_products` (`status`);

CREATE INDEX `bc_products_index_21` ON `bc_products` (`sync_status`);

CREATE UNIQUE INDEX `bc_brands_index_22` ON `bc_brands` (`store_id`, `bc_brand_id`);

CREATE UNIQUE INDEX `bc_product_categories_index_23` ON `bc_product_categories` (`product_id`, `category_id`);

CREATE UNIQUE INDEX `bc_orders_index_24` ON `bc_orders` (`store_id`, `bc_order_id`);

CREATE INDEX `bc_orders_index_25` ON `bc_orders` (`store_id`);

CREATE INDEX `bc_orders_index_26` ON `bc_orders` (`bc_order_id`);

CREATE INDEX `bc_orders_index_27` ON `bc_orders` (`status`);

CREATE INDEX `bc_orders_index_28` ON `bc_orders` (`created_at`);

CREATE INDEX `bc_order_items_index_29` ON `bc_order_items` (`order_id`);

CREATE INDEX `bc_order_items_index_30` ON `bc_order_items` (`product_id`);

CREATE INDEX `bc_inventory_transactions_index_31` ON `bc_inventory_transactions` (`product_id`);

CREATE INDEX `bc_inventory_transactions_index_32` ON `bc_inventory_transactions` (`created_at`);

CREATE INDEX `bc_webhook_events_index_33` ON `bc_webhook_events` (`processed`);

CREATE INDEX `bc_webhook_events_index_34` ON `bc_webhook_events` (`created_at`);

CREATE INDEX `bc_sync_jobs_index_35` ON `bc_sync_jobs` (`status`);

CREATE INDEX `bc_sync_jobs_index_36` ON `bc_sync_jobs` (`created_at`);
...
ALTER TABLE `kit_info` ADD FOREIGN KEY (`stock_item_id`) REFERENCES `stock_item` (`stock_item_id`);

ALTER TABLE `kit_info` ADD FOREIGN KEY (`belongs_to`) REFERENCES `users` (`user_id`);

ALTER TABLE `stock_item_map_kit` ADD FOREIGN KEY (`kit_id`) REFERENCES `kit_info` (`id`);

ALTER TABLE `stock_item_map_kit` ADD FOREIGN KEY (`component_stock_item_id`) REFERENCES `stock_item` (`stock_item_id`);
...
ALTER TABLE `images` ADD FOREIGN KEY (`kit_id`) REFERENCES `kit_info` (`id`);
