...
interface ChannelLinkViewProps {
  id: string;
  channelName: string;
  channelType: string;
  product: any;
...
const channelIcons: Record<string, { icon: string; color: string; bgColor: string; borderColor: string }> = {
  shopify: {
  ...
  woocommerce: {
    icon: "https://www.vectorlogo.zone/logos/woocommerce/woocommerce-icon.svg",
    color: "purple",
...
export default function ChannelLinkView({ id, channelName, channelType, product }: ChannelLinkViewProps) {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedProductId, setSelectedProductId] = useState<string | null>(null);
  const [isLinking, setIsLinking] = useState(false);

  const channelConfig = channelIcons[channelType.toLowerCase()] || channelIcons.default;
  ...
  const topBarColor = channelConfig.color === "emerald" ? "bg-emerald-500" :
    channelConfig.color === "orange" ? "bg-orange-500" :
      channelConfig.color === "blue" ? "bg-blue-500" :
        channelConfig.color === "purple" ? "bg-purple-500" :
          channelConfig.color === "indigo" ? "bg-indigo-500" :
            "bg-gray-500";

  const labelColor = channelConfig.color === "emerald" ? "text-emerald-600" :
    channelConfig.color === "orange" ? "text-orange-600" :
      channelConfig.color === "blue" ? "text-blue-600" :
        channelConfig.color === "purple" ? "text-purple-600" :
          channelConfig.color === "indigo" ? "text-indigo-600" :
            "text-gray-600";

  return (
    <div className="flex flex-col xl:flex-row gap-8 animate-fadeIn h-[calc(100vh-12rem)] min-h-[600px]">

      {/* LEFT: External Channel Node */}
      <div className="flex-1 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 flex flex-col relative overflow-hidden group">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className={`w-12 h-12 rounded-2xl ${channelConfig.bgColor} flex items-center justify-center p-2.5 border ${channelConfig.borderColor}`}>
            {channelConfig.icon ? (
              <Image
              ...
              <svg className={`w-6 h-6 ${labelColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div>
            <p className={`text-[10px] font-black ${labelColor} uppercase tracking-widest mb-1`}>External Channel Node</p>
            <h2 className="text-xl font-bold text-gray-800 tracking-tight">{channelName}</h2>
          </div>
        </div>
        ...
        <div className="space-y-8 flex-1">
          ...
          <div className="p-4 rounded-xl bg-orange-50 border border-orange-100 flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-600 leading-relaxed font-medium">This product exists on {channelName} but is not connected to a Central Inventory item. Stock updates will not sync.</p>
            </div>
          </div>
        </div>
      </div>
