...
function BigCommerceListingEditLoader() {
  const { id } = useParams();
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          const formData: ProductFormData = {
            ...
            marketplace: {
              channels: [{
                id: listing.channel?.marketplace || "bigcommerce",
                name: listing.channel?.name || "BigCommerce",
  ...
  if (mode === 'link' && product) {
    ...
    return (
      <ChannelLinkView
        id={id as string}
        channelName={product.channel?.name || "BigCommerce Channel"}
...
export default function EditBigCommerceListingPage() {
  return (
    <div className="p-3 sm:p-4 lg:p-4 w-full max-w-[1600px] mx-auto font-outfit select-none">
      <div className="mb-4">
        <nav className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-400 mb-2">
          ...
          <span className="text-gray-200">/</span>
          <span className="text-indigo-600">BigCommerce Channel</span>
        </nav>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-700 tracking-tight">Edit BigCommerce Inventory</h1>
