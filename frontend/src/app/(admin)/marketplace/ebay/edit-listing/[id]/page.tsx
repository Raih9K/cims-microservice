...
function EbayListingEditLoader() {
  const { id } = useParams();
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          const formData: ProductFormData = {
            ...
            marketplace: {
              channels: [{
                id: listing.channel?.marketplace || "ebay",
                name: listing.channel?.name || "eBay",
  ...
  if (mode === 'link' && product) {
    ...
    return (
      <ChannelLinkView
        id={id as string}
        channelName={product.channel?.name || "eBay Channel"}
        channelType="ebay"
        product={viewProduct}
      />
    );
...
export default function EditEbayListingPage() {
  return (
    <div className="p-3 sm:p-4 lg:p-4 w-full max-w-[1600px] mx-auto font-outfit select-none">
      <div className="mb-4">
        <nav className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-400 mb-2">
          ...
          <span className="text-gray-200">/</span>
          <span className="text-blue-600">eBay Channel</span>
        </nav>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-700 tracking-tight">Edit eBay Inventory</h1>
